...
//              Debug.Log ("markers " + markers  .Count);
        ...
        void findMarkers (Mat bgraMat, List<Marker> detectedMarkers)
        {
            ...
            //Imgproc.threshold(m_grayscaleImage, m_thresholdImg, 127, 255, Imgproc.THRESH_BINARY_INV | Imgproc.THRESH_OTSU);
            Debug.Log("Doing threshold Job:"+ MarkerDec.myFilter1 + MarkerDec.myFilter2);
            Imgproc.threshold(m_grayscaleImage, m_thresholdImg, MarkerDec.myFilter1, MarkerDec.myFilter2, Imgproc.THRESH_BINARY_INV | Imgproc.THRESH_OTSU);


            //Filter before Adaptive Threshold
            //Imgproc.medianBlur(m_grayscaleImage, m_grayscaleImage, 5);
            //Imgproc.GaussianBlur(m_grayscaleImage, m_grayscaleImage, new Size(5, 5), 0);

            Imgproc.bilateralFilter(m_grayscaleImage, m_grayscaleImage, 10, 50, 50);


            //Imgproc.adaptiveThreshold(m_grayscaleImage, m_thresholdImg, 255, Imgproc.ADAPTIVE_THRESH_MEAN_C, Imgproc.THRESH_BINARY_INV, 11, 2);
            Imgproc.adaptiveThreshold(m_grayscaleImage, m_thresholdImg, 255, Imgproc.ADAPTIVE_THRESH_GAUSSIAN_C, Imgproc.THRESH_BINARY_INV, 7, 2);
